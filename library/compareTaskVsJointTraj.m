...
close all
figure, hold on
plot3(posTask(1,:),posTask(2,:),posTask(3,:),'b-','Linewidth',lw);
plot3(posJoint(1,:),posJoint(2,:),posJoint(3,:),'k--','Linewidth',lw);
...
for idx = 1:numJoints
    ...
    plot(trajTimes,qTask(idx,:),'b-','Linewidth',lw);
    plot(trajTimes,qJoint(idx,:),'k-','Linewidth',lw);
    for wIdx = 1:numWaypoints
       xline(waypointTimes(wIdx),'r--'); 
    end
